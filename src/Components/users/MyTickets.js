...
const MyTickets = () => {
  ...
  const handleExchangeRequest = async (ticketId) => {
    const storedUser = localStorage.getItem("userInfo");
    if (!storedUser) {
      setError("Bạn chưa đăng nhập.");
      return;
    ...
    if (!ticketId) {
      setError("Không tìm thấy ID vé để đổi.");
    ...
    try {
      console.log("Sending exchange request with data:", {
        requesterId: userId,
        requestedTicketId: ticketId,
        message: exchange.message,
      });

      const response = await axios.post("https://exe202-backend-2v40.onrender.com/api/exchange/exchange-requests", {
        requesterId: userId,
        requestedTicketId: ticketId,
        message: exchange.message,
      });
      ...
      console.error("Exchange request error:", err.response?.data || err);
            ...
            >
              <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                    ...
                    <div className="flex items-center">
                      ...
                      <p className="text-sm">
                        {ticket.route.departureTimes?.length > 0
                          ? ticket.route.departureTimes[0].split("T")[0] // Lấy phần ngày
                          : "Chưa có thông tin"}
                      </p>
                    </div>
                    <div className="flex items-center">
                      ...
                      <p className="text-sm">
                        {ticket.route.departureTimes?.length > 0
                          ? ticket.route.departureTimes[0].split("T")[1]?.substring(0, 5) // Lấy phần giờ:phút
                          : "Chưa có giờ"}
                      </p>
                    </div>
                    <div className="flex items-center">
                      ...
                      <p className="text-sm">
                        Ghế: {Array.isArray(ticket.seatNumbers)
                ...
                <div className="md:w-36 flex md:flex-col gap-2">
                  ...
                  <button
                    onClick={() => {
                      if (!ticket.ticketId) { // Thay _id bằng ticketId
                        console.error("Ticket does not have a ticketId:", ticket);
                        setError("Vé không có ID hợp lệ.");
                        return;
                      }
                      console.log("Setting exchange for ticket:", ticket.ticketId);
                      setExchange({ ticketId: ticket.ticketId, message: "", loading: false }); // Thay _id bằng ticketId
                    }}
              ...
              {exchange.ticketId === ticket.ticketId && ( // Thay _id bằng ticketId
                <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                  ...
                  <div className="flex justify-end mt-3">
                    ...
                    <button
                      onClick={() => handleExchangeRequest(ticket.ticketId)} // Thay _id bằng ticketId
                  ...
                  <div className="flex flex-col gap-2 mb-6">
                    ...
                    <div className="flex flex-col sm:flex-row sm:gap-8">
                      <div className="flex items-center gap-2 text-gray-700">
                        <Calendar size={16} />
                        <span>{selectedTicket.departureTimes?.split("T")[0] || "N/A"}</span> {/* Lấy phần ngày */}
                      </div>
                      <div className="flex items-center gap-2 text-gray-700">
                        <Clock size={16} />
                        <span>{selectedTicket.departureTimes?.split("T")[1]?.substring(0, 5) || "N/A"}</span> {/* Lấy phần giờ:phút */}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-lg font-bold">
                          {Array.isArray(selectedTicket.seatNumbers)
                            ? selectedTicket.seatNumbers.join(", ")
                            : selectedTicket.seatNumbers}
                        </span>
                      </div>
                    </div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                ...
                <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 mb-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="font-medium">{selectedTicket?.createdAt?.split("T")[0] || "N/A"}</div> {/* Lấy phần ngày */}
                    </div>
                    <div>
                      <div className="text-sm text-gray-500">Giờ đặt vé</div>
                      <div className="font-medium">{selectedTicket?.createdAt?.split("T")[1]?.substring(0, 5) || "N/A"}</div> {/* Lấy phần giờ:phút */}
                    </div>
                    <div>
